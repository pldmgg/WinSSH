function Uninstall-WinSSH {
    ...
    if (!$(Test-Path "$OpenSSHProgramFilesPath\uninstall-sshd.ps1")) {
        if (!$(Test-Path $OpenSSHProgramFilesPath)) {
            $null = New-Item -ItemType Directory -Path $OpenSSHProgramFilesPath
        }
        $UninstallSSHAgentAndSSHDScript = @'
...
'@ | Set-Content "$OpenSSHProgramFilesPath\uninstall-sshd.ps1"
    }

    try {
        & "$OpenSSHProgramFilesPath\uninstall-sshd.ps1"
    }
    catch {
        Write-Error $_
        $global:FunctionResult = "1"
    ...
    if (Test-Path "C:\ProgramData\ssh") {
        try {
            Remove-Item "C:\ProgramData\ssh" -Recurse -Force
        }
        catch {
            Write-Error $_
            $global:FunctionResult = "1"
