function InstallProgram {
    ...
    if (!$NoUpdatePackageManagement) {
        ...
        try {
            ...
            Write-Error $_
            Write-Host "Errors from the UpdatePackageManagement function are as follows:"
    ...
    $env:Path = RefreshChocolateyEnv -ErrorAction SilentlyContinue
    ...
    $ChocolateyOutdatedProgramsPSObjects.ProgramName -contains $ProgramName
        ...
        $PackageManagementInstalledPrograms.Name -contains $ProgramName -and $ChocolateyInstalledProgramsPSObjects.ProgramName -notcontains $ProgramName
            ...
            if ($InstallError.Count -gt 0) {
                ...
                Synchronize-SystemPathEnvPath
                $env:Path = $($(RefreshChocolateyEnv -ErrorAction SilentlyContinue) -split ";" | foreach {
        ...
        $ChocolateyInstalledProgramsPSObjects.ProgramName -contains $ProgramName
            ...
            try {
                ...
                $global:FunctionResult = "0"
                $env:Path = RefreshChocolateyEnv -ErrorAction SilentlyContinue -ErrorVariable RCEErr

                # The first time we attempt to RefreshChocolateyEnv, Chocolatey CmdLine and/or the
                # Chocolatey Package Provider legitimately might not be installed,
                # so if the RefreshChocolateyEnv function throws that error, we can ignore it
                ...
                ![bool]$($RCEErr -match "Neither the Chocolatey PackageProvider nor the Chocolatey CmdLine appears to be installed!")) {
                    throw "The RefreshChocolateyEnv function failed! Halting!"
                }
            }
            catch {
                Write-Error $_
                Write-Host "Errors from the RefreshChocolateyEnv function are as follows:"
            ...
            if (![bool]$(Get-Command choco -ErrorAction SilentlyContinue)) {
                try {
                    $global:FunctionResult = "0"
                    $null = InstallChocolateyCmdLine -NoUpdatePackageManagement -ErrorAction SilentlyContinue -ErrorVariable ICCErr
                    if ($ICCErr -and $global:FunctionResult -eq "1") {throw "The InstallChocolateyCmdLine function failed! Halting!"}
                }
                catch {
                    Write-Error $_
                    Write-Host "Errors from the InstallChocolateyCmdLine function are as follows:"
        ...
        if ($ResolveCommandPath -or $PSBoundParameters['CommandName']) {
            ...
            if (![bool]$(Get-Command $FinalCommandName -ErrorAction SilentlyContinue)) {
                try {
                    ...
                    $global:FunctionResult = "0"
                    $env:Path = RefreshChocolateyEnv -ErrorAction SilentlyContinue -ErrorVariable RCEErr
                    if ($RCEErr.Count -gt 0 -and $global:FunctionResult -eq "1") {throw "The RefreshChocolateyEnv function failed! Halting!"}
                }
                catch {
                    Write-Error $_
                    Write-Host "Errors from the RefreshChocolateyEnv function are as follows:"
